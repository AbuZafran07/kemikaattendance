...
interface CalcInput {
  ...
  /**
   * Bila diisi, helper memvalidasi dokumen CA (Cash Advance) pada pengajuan ini.
   * Tanpa dokumen CA, tunjangan perjalanan dinas TIDAK diproses.
   */
  requestId?: string;
}
...
export async function applyBusinessTravelAllowance(input: CalcInput): Promise<BusinessTravelAllowanceResult> {
  const { userId, startDate, endDate, dryRun = false, requestId } = input;
  const start = parseISO(startDate);
  const end = parseISO(endDate);

  // 0. Validasi dokumen CA (Cash Advance) — syarat wajib keluarnya tunjangan dinas
  if (requestId) {
    const { data: reqRow } = await supabase
      .from("business_travel_requests")
      .select("ca_document_url")
      .eq("id", requestId)
      .maybeSingle();

    if (!(reqRow as any)?.ca_document_url) {
      return {
        ok: false,
        amount: 0,
        period_month: start.getMonth() + 1,
        period_year: start.getFullYear(),
        travel_days_effective: 0,
        per_day_attendance: 0,
        per_day_travel: 0,
        reason:
          "Tunjangan dinas tidak diproses: dokumen CA (Cash Advance) belum dilampirkan pada pengajuan ini.",
      };
    }
  }
