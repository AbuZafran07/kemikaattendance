 ...
 */
export async function exportMultiSheetExcelFile(
  sheets: { sheetName: string; data: Record<string, any>[]; headerRows?: string[][] }[],
  fileName: string,
) {
  const workbook = new ExcelJS.Workbook();

  for (const { sheetName, data, headerRows } of sheets) {
    const worksheet = workbook.addWorksheet(sheetName);

    if (headerRows && headerRows.length > 0) {
      for (const row of headerRows) worksheet.addRow(row);
      worksheet.addRow([]);
    }
